    ...
    <div class="space-y-2 lg:space-y-0 lg:space-x-4 mt-8 lg:inline-flex">
        ...
        <div class="relative flex lg:inline-flex items-center bg-white">
            <div x-data="{show: false}" @click.away="show = false">
                <button @click="show = !show" class='py-2 pl-3 pr-9 text-sm font-semibold w-full lg:w-32 text-left flex lg:inline-flex'>{{ isset($currentSeller) ? ucwords($currentSeller->name) : 'Sellers' }}
                ...
                <div x-show="show" class="border-black border py-2 absolute bg-white mt-2 w-full z-50" style="display:none">
                    ...
                    <a href="/authors/{{ $seller->username }}" class="block text-left px-3 text-sm leading-6 hover:bg-primary focus:bg-primary hover:text-white focus {{ isset($currentSeller) && $currentSeller->id == $user->id ? 'bg-blue-500 text-white' : '' }}">
                        {{ ucwords($seller->name) }}
                    </a>
                    @endforeach
                </div>
